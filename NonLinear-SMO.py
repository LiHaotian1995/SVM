...
# a data structure
class optStruct:
    def __init__(self, dataMatIn, labelMatIn, C, tolerance, kernelFunction):
        ...
        self.kMatrix = np.mat(np.zeros((self.m, self.m)))
        for i in range(self.m):
            self.kMatrix[:,i] = kernelTrans(self.dataMat, self.dataMat[i,:], kernelFunction)


# the kernel function (use RBF)
def kernelTrans(dataMat1, dataMat2, kernelFunction):
    m, n = np.shape(dataMat1)
    k = np.mat(np.zeros((m,1)))
    if kernelFunction[0] == 'line':
        k = dataMat1 * dataMat2.transpose()
    # you can add more elif to add other function about the kernel (like gauss function...)
    elif kernelFunction[0] == 'rbf':
        for j in range(m):
            deltaRow = dataMat1[j,:] - dataMat2
            k[j] = deltaRow * deltaRow.transpose()
        k = np.exp(k / (-1 * kernelFunction[1] ** 2))
    else:
        raise NameError('Error! The kernel function is not recognized')
...
def loadDataSet():
    dataMat = []; labelMat = []
    f = open('/Users/lihaotian/PycharmProjects/SVM/testSetRBF2.txt')
...
def plotOriginalDataSet(dataMat, labelMat):
    dataMat1 = []; dataMatNo1 = []
...
def calculateEk(oS, k):
    fXk = float(np.multiply(oS.alpha, oS.labelMat).transpose() * oS.kMatrix[:, k]) + oS.b
...
def innerSMO(i, oS):
    ...
    if ((oS.labelMat[i]*Ei < - oS.tolerance) and (oS.alpha[i] < oS.C)) or\
        ...
        eta = oS.kMatrix[i, i] + oS.kMatrix[j, j] - 2.0 * oS.kMatrix[i, j]
        if (eta <= 0):
            print("\n eta <= 0")
            return 0
        ...
        newBi = oS.b - Ei - oS.labelMat[i] * (oS.alpha[i] - alphaOldI) * oS.kMatrix[i, i] - \
                oS.labelMat[j] * (oS.alpha[j] - alphaOldJ) * oS.kMatrix[i, j]

        newBj = oS.b - Ej - oS.labelMat[i] * (oS.alpha[i] - alphaOldI) * oS.kMatrix[i, j] - \
                oS.labelMat[j] * (oS.alpha[j] - alphaOldJ) * oS.kMatrix[j, j]
...
def SMO(dataMatIn, labelMatIn, C, tolerance, maxIter, kernelFunction = ('line', 0)):
    ...
    oS = optStruct(np.mat(dataMatIn), np.mat(labelMatIn).transpose(), C, tolerance, kernelFunction)
...
if __name__ == '__main__':
    ...
    alpha, b = SMO(dataMat, labelMat, 0.6, 0.001, 40, ('rbf', 1.8))
    print("\n alpha: \n" + str(alpha))
    print("\n b = " + str(float(b)))
    ...
    # plotOriginalDataSet(dataMat, labelMat)
